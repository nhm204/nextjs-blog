...
const PostDetail = ({ post }) => {
  const getContentFragment = (index, text, obj, type) => {
    ...
    switch (type) {
      ...
      case 'paragraph':
        return <p key={index} className="mb-4 last:mb-0">{modifiedText.map((item, i) => <React.Fragment key={i}>{item}</React.Fragment>)}</p>;
      case 'heading-four':
    ...
    <>
      <div className="bg-white shadow-lg rounded-lg lg:p-8 pb-12 mb-8">
        ...
        <div className="px-4 lg:px-0">
          ...
          <h1 className="mb-20 text-6xl font-semibold">{post.title}</h1>
